...
Q1. The functionality of this assembly code is to initialize three variables, calculate (3X5 - 3/2) and store the result in the last variable. The result is 14.\
...
Q3. The functionality of this assembly code is to find values ranging from 100 to 999, which satisfies the condition x = (x/100)^3 + (((x/100)X(-100)+x)/10)^3 +  (x - (x/10 + x/10X4)X2)^3. My c program returns 4 results, which are 153, 370, 371, and 407.\
